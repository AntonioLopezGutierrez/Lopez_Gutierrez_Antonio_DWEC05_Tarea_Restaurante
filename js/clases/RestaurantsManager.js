...
class RestaurantsManager {
    ...
    constructor() {
        //Comprobacion de que no se pueden crear mas de una instancia de la clase
        ...
        this.#menus = [];
        this.#restaurants = [];
        //Asignamos la instancia del primer objeto a la propiedad #instance
        RestaurantsManager.#instance = this;
    }
    ///////////////////////////////////////////////GETTER Y SETTER/////////////////////////////////////////////
    //Getter para instanciar objetos de la clase que seran la misma instancia
    static getInstance() {
    ...
    createCategory(name, description) {
        ...
        let categoriasEncontradas = this.#categories.filter(categories => categories.categoria.getName() == name);
    ...
    addCategory(...categorias) {
        ...
        for (const categoria of categorias) {
            ...
            let categoriasEncontradas = this.#categories.filter(categorias =>
                categorias.categoria.getName() === categoria.categoria.getName());
    ...
    removeCategory(...categorias) {
        ...
        for (const categoria of categorias) {
            ...
            //Utilizamos el metodo filter para encontrar la categoria con el nombre dado
            let categoriasRestantes = this.#categories.filter(categorias => categorias.categoria.getName()
    ...
    assignCategoryToDish(...dishesAndCategories) {
        ...
        for (const platosCategorias of dishesAndCategories) {
            ...
            if (platosCategorias.platos instanceof Dish) {
                ...
                let existePlato = this.#dishes.find(dish => dish.platos.getName() === platosCategorias.platos.getName());
            ...
            if (platosCategorias.categoria instanceof Category) {
                ...
                let existeCategoria = this.#categories.find(category => category.categoria.getName()
                    === platosCategorias.categoria.getName());
        ...
        for (let i = 0; i < categoriasEntrante.length; i++) {
            ...
            let existingAssignment = this.#categories.find(categoriaPlatoObj =>
                categoriaPlatoObj.categoria.getName() === category.categoria.getName());

            //Buscamos si el plato ya esta asignado a esa categoria
            let platoExistente = existingAssignment.platos.find(existingPlato =>
                existingPlato.platos.getName() === dish.platos.getName());
    ...
    deassignCategoryToDish(category, dish) {
        ...
        let existingCategory = this.#categories.find(existingCategory =>
        ...
        let existingDish = this.#dishes.find(existingDish =>
            existingDish.platos.getName() === dish.platos.getName());
        ...
        existingCategory.platos = existingCategory.platos.filter(existingDish =>
            existingDish.platos.getName() !== dish.platos.getName());
    ...
    getDishesInCategory(category, funcionOrdenado) {
        ...
        let existeCategoria = this.#categories.find(categoria => categoria.categoria.getName()
    ...
    addMenu(...menus) {
        ...
        for (const menu of menus) {
            ...
            let menuEncontrados = this.#menus.filter(menus => menus.menus.getName() === menu.menus.getName());
    ...
    removeMenu(...menus) {
        ...
        for (const menu of menus) {
            ...
            let menusRestantes = this.#menus.filter(menus => menus.menus.getName() !== menu.menus.getName());
    ...
    assignDishToMenu(...dishesAndMenu) {
        ...
        for (const platosMenu of dishesAndMenu) {
            ...
            if (platosMenu.platos instanceof Dish) {
                ...
                //Buscamos el plato por el nombre
                let existePlato = this.#dishes.find(dish => dish.platos.getName() === platosMenu.platos.getName());
                //Si no existe el plato lo añadimos a la la lista de platos
                if (!existePlato) {
            ...
            if (platosMenu.menus instanceof Menu) {
                ...
                let existeMenu = this.#menus.find(menus => menus.menus.getName()
                    === platosMenu.menus.getName());
        ...
        for (let i = 0; i < menusEntrante.length; i++) {
            ...
            let existingAssignment = this.#menus.find(exitingMenu =>
                exitingMenu.menus.getName() === menu.menus.getName());
    ...
    deassignDishToMenu(menu, dish) {
        ...
        let existingMenu = this.#menus.find(existingMenu =>
            existingMenu.menus.getName() === menu.menus.getName());
        ...
        existingMenu.platos = existingMenu.platos.filter(existingDish =>
            existingDish.platos.getName() !== dish.platos.getName());
    ...
    changeDishesPositionsInMenu(menu, dish1, dish2) {
        ...
        let posicion1 = existingMenu.platos.findIndex(existingDish =>
            existingDish.platos.getName() === dish1.platos.getName());

        let posicion2 = existingMenu.platos.findIndex(existingDish =>
            existingDish.platos.getName() === dish2.platos.getName());
        ...
        [existingMenu.platos[posicion1], existingMenu.platos[posicion2]] =
            [existingMenu.platos[posicion2], existingMenu.platos[posicion1]];
    ...
    createAllergen(name, description) {
        ...
        let alergenosEncontrados = this.#allergens.filter(allergens => allergens.getName() == name);
    ...
    addAllergen(...allergens) {
        ...
        for (const allergen of allergens) {
            ...
            let allergenEncontrados = this.#allergens.filter(allergens => allergens.getName() === allergen.getName());
    ...
    removeAllergen(...allergens) {
        ...
        for (const allergen of allergens) {
            ...
            let allergenRestantes = this.#allergens.filter(allergens => allergens.getName() !== allergen.getName());
    ...
    createRestaurant(name, description, location) {
        ...
        let restaurantesEncontrados = this.#restaurants.filter(restaurants => restaurants.getName() == name);
    ...
    addRestaurant(...restaurants) {
        ...
        for (const restaurant of restaurants) {
            ...
            let restaurantEncontrados = this.#restaurants.filter(restaurants => restaurants.getName() === restaurant.getName());
    ...
    removeRestaurant(...restaurants) {
        ...
        for (const restaurant of restaurants) {
            ...
            let restaurantRestantes = this.#restaurants.filter(restaurants => restaurants.getName() !== restaurant.getName());
    ...
    addDish(...dishes) {
        ...
        for (const dish of dishes) {
            ...
            let dishEncontrados = this.#dishes.filter(dishes => dishes.platos.getName() === dish.platos.getName());
    ...
    removeDish(...dishes) {
        ...
        for (const dish of dishes) {
            ...
            let existingDish = this.#dishes.find(existing => existing.platos.getName()
                === dish.platos.getName());
            ...
            this.#categories.forEach(category => {
                ...
                category.platos = category.platos.filter(existing => existing.platos.getName()
                    !== dish.platos.getName());
            });
            ...
            existingDish.alergenos.forEach(alergeno => {
                ...
                let allergenIndex = this.#allergens.findIndex(existingAllergen =>
                    existingAllergen.getName() === alergeno.getName());
            ...
            this.#menus.forEach(menu => {
                menu.platos = menu.platos.filter(existing => existing.platos.getName() !== dish.platos.getName());
            });

            //Eliminamos el plato de la lista principal de platos
            this.#dishes = this.#dishes.filter(existing => existing.platos.getName() !== dish.platos.getName());
        }
    ...
    assignAllergenToDish(...dishesAndAllergen) {
        ...
        for (const platosAlergenos of dishesAndAllergen) {
            ...
            if (platosAlergenos.platos instanceof Dish) {
                ...
                //Buscamos el plato por el nombre
                let existePlato = this.#dishes.find(dish => dish.platos.getName() === platosAlergenos.platos.getName());
                //Si no existe el plato lo añadimos a la la lista de platos
                if (!existePlato) {
            ...
            if (platosAlergenos instanceof Allergen) {
                ...
                let existeAlergeno = this.#allergens.find(allergens => allergens.getName()
                    === platosAlergenos.getName());
        ...
        for (let i = 0; i < platosEntrantes.length; i++) {
            ...
            //Buscamos el plato en las asignaciones existentes por el nombre
            let existingAssignment = this.#dishes.find(dishes =>
                dishes.platos.getName() === dish.platos.getName());
            //Buscamos si el alergeno ya esta asignado al plato
            let alergenoExistente = existingAssignment.alergenos.find(existingAlergeno =>
                existingAlergeno.getName() === allergen.getName());
    ...
    deassignAllergenToDish(dish, allergen) {
        ...
        //Buscamos la categoria en las asignaciones existentes por el nombre
        let existingPlato = this.#dishes.find(existingDish =>
            existingDish.platos.getName() === dish.platos.getName());
        ...
        let existingAllergen = this.#allergens.find(existingAllergen =>
            existingAllergen.getName() === allergen.getName());
        ...
        existingPlato.alergenos = existingPlato.alergenos.filter(existingAllergen =>
            existingAllergen.getName() !== allergen.getName());
    ...
    getDishesWithAllergen(allergen, funcionOrdenado) {
        ...
        let existeAlergeno = this.#allergens.find(existing => existing.getName() === allergen.getName());
    ...
    findDishes(dish, callback, funcionOrdenadoMenus) {
        ...
        //Buscamos el plato por el nombre
        let existePlato = this.#dishes.find(plato => plato.platos.getName() === dish.platos.getName());
    ...
    toString() {
        ...
        let categorias = this.#categories.map(function (categoriaPlato) {
            //Obtenemos el nombre de la categoria
            const categoriaAsignada = categoriaPlato.categoria.getName();

            //Iteramos sobre los platos asignados a la categoria
            const platoAsignados = categoriaPlato.platos.map(function (platoAlergeno) {
            ...
            return "Categoria: " + categoriaAsignada + "\nPlatos asignados:\n" + platoAsignados;
        }).join("");

        //Desglose del array de platos
        let platos = this.#dishes.map(function (platoAlergeno) {
            //Obtenemos el nombre del plato
            const platosAsignados = platoAlergeno.platos.getName();

            //Iteramos sobre los platos asignados a la categoria
            const alergenosAsignados = platoAlergeno.alergenos.map(function (alergeno) {
        ...
        return (
            "El nombre del sistema es: " + this.#nombre + "\n" +
            "Las categorías del sistema son: \n" + categorias + "\n" +
            "Los alergenos del sistema son: \n" + alergenos + "\n" +
            "Los platos del sistema son: \n" + platos + "\n" +
            "Los menus del sistema son: \n" + menus + "\n" +
            "Los restaurantes del sistema son: \n" + restaurantes
        );
    }
